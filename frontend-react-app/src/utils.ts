 ...
 */
export const readObjectFromFile = (
  path: string | undefined
): any | undefined => {
...
export const readAllContractsFromDir = (
  ...paths: string[]
  ...
  for (let i = 0; i < paths.length; i++) {
    let path = paths[i];
    if (path && fs.existsSync(path)) {
      const deployContents: fs.Dirent[] = fs.readdirSync(path, {
        withFileTypes: true,
      });
      deployContents.forEach((deployEntry) => {
        if (deployEntry.isFile()) {
          ...
          if (filename.endsWith(".json") && filename !== "dapp.json") {
            const contractName = filename.substring(
              0,
              filename.lastIndexOf(".")
            );
            contracts[contractName] = readContractFromFile(
              `${path}/${filename}`
            );
          }
        }
