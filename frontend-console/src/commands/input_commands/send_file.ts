...
export const command = "send_file";
export const describe = "Send file as input to DApp";
...
export const builder = (yargs: Argv<{}>): Argv<Args> => {
    ...
    // this command args
    return rollupsArgs.option("path", {
        describe: "Input path of file to send",
        type: "string",
        demandOption: true,
    });
};
...
export const handler = async (args: Args) => {
    const { rpc, path, mnemonic, accountIndex } = args;
    ...
    const inputBytes = fs.readFileSync(path);

    //const inputBytes = ethers.utils.toUtf8Bytes(file.toString("base64"));
